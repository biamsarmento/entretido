  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="flex items-center justify-between px-6 py-4 border-b border-muted">
        <Image src="/brand/logo.png" alt="Entretido" width={180} height={100} className="object-cover" />
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center px-6 py-24">
        <div className="max-w-2xl">
          ...
          <p className="text-xl text-muted-foreground mb-10 leading-relaxed">
            Crie grupos com seus amigos, compartilhe recomendações e organize
            tudo que você quer assistir em um só lugar.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link
              href="/signup"
              className="px-8 py-3 bg-primary hover:bg-primary-hover text-white rounded-lg font-medium text-lg transition-colors"
            >
              Começar grátis
            </Link>
            <Link
              href="/login"
              className="px-8 py-3 border border-muted hover:border-border text-muted-foreground hover:text-foreground rounded-lg font-medium text-lg transition-colors"
            >
              Já tenho conta
            </Link>
          </div>
          ...
          {[
            {
              ...
              desc: "Crie grupos e convide quem quiser com um código simples.",
            },
            {
              icon: <Film size={24} className="text-primary" />,
              title: "Busca inteligente",
              desc: "Encontre qualquer filme ou série com a base do TMDB.",
            },
            {
              icon: <Bookmark size={24} className="text-primary" />,
              title: "Ver mais tarde",
              desc: "Salve o que te interessou e acesse quando quiser.",
            },
            {
              icon: <Star size={24} className="text-primary" />,
              title: "Quem indicou",
              desc: "Veja qual amigo recomendou cada título no grupo.",
            },
          ].map((f) => (
            <div
              key={f.title}
